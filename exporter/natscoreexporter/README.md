...
- `traces`
  - `subject` (default = "otel_spans"): The [OTTL value expression](https://pkg.go.dev/text/template) used to construct NATS subjects for traces.
...
The following authentication options are set directly on the exporter (they are not nested under an `auth` key). At most one may be configured:
